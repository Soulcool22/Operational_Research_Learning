...
这个模块用4个函数解决生产计划问题：建模求解、可视化、敏感性分析、报告生成。
...
**通俗解释**：
- `LpProblem()`：创建优化问题，告诉程序我们要最大化还是最小化
- `LpVariable()`：决策变量，我们要决定的数值（生产多少）
- `lpSum()`：求和函数，把多个项加起来
- `lowBound=0`：变量下界，生产数量不能是负数
- `.varValue`：获取变量的最优值
...
**通俗解释**：
- `bar()`：画柱状图，每个产品一根柱子
- `color=['#FF6B6B', '#4ECDC4', '#45B7D1']`：设置柱子颜色，让图表更美观
- `text()`：在柱子上添加数字标签
- `dpi=300`：高分辨率保存，图片更清晰
...
**通俗解释**：
- 敏感性分析：看看参数变化对结果的影响有多大
- `copy()`：复制列表，避免修改原始数据
- `{change:+d}%`：格式化输出，显示正负号
- 重新建模：每次分析都要重新求解，确保结果准确
...
这个线性规划模块把复杂的数学优化变成了实用的决策工具，帮助解决现实中的资源分配和计划优化问题！
