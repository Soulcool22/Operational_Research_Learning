...
- `contourf()`：填充函数，将满足条件的区域着色显示。
- 等高线：同一条线上的点具有相同的目标函数值。
...
**说明**：
- `learning_rate`：学习率，控制每次迭代步长；过大可能震荡，过小则收敛缓慢。
- `gradient()`：梯度指向函数增长最快的方向，负梯度指向下降最快的方向。
- 路径可视化：直观展示算法向最优点收敛的过程。
...
- `flow_ratio`：流量占容量的比例，用于控制线条粗细和颜色深浅。
- 可视化效果：粗线表示流量较大，细线表示流量较小。
...
**说明**：
- 敏感性分析：衡量参数变化对结果的影响程度。
- 影子价格：增加一单位资源带来的目标函数改善。
- 热力图：颜色深浅表示目标函数值大小。
...
- `np.where(constraint_mask, Z3, np.nan)`：只显示满足约束的区域。
- 帕累托前沿：多目标优化中，改善一个目标需相应牺牲另一目标。
...
**说明**：
- 收敛曲线：用于比较算法接近最优解的速度。
- 雷达图：多维度对比算法性能，面积越大表示综合表现更好。
- 对数坐标：在数值差异较大时便于观察变化趋势。
